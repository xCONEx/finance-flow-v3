...
const AddReserveGoalModal: React.FC<AddReserveGoalModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Using expenses table temporarily to store reserve goals
      ...
      const { error } = await supabase
        ...
        .insert({
          user_id: user.id,
          description: `RESERVE_GOAL: ${formData.name} | Target: ${formData.target_amount} | Icon: ${formData.icon || '🎯'} | Current: 0`,
          value: 0, // Initial current amount
          category: 'Meta de Reserva',
          month: currentMonth
        });
...
<div className="space-y-2">
  ...
  <Input
    ...
    onChange={(e) =>
      setFormData({
        ...formData,
        target_amount: e.target.value === '' ? '' : parseFloat(e.target.value),
      })
    }
    required
  />
